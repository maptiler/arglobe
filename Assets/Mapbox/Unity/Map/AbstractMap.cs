...
namespace Mapbox.Unity.Map
{
	...
	public class AbstractMap : MonoBehaviour, IMap
	{
		...
		private void OnEnable()
		{
			...
			{
				_options.tileMaterial = new Material(Shader.Find("Universal Render Pipeline/Unlit"));
				_options.tileMaterial.SetFloat("_Cull", 0);
			}
		...
		protected virtual void InitializeMap(MapOptions options)
		{
			...
			_options.locationOptions.PropertyHasChanged += (object sender, EventArgs eventArgs) =>
			{
				UpdateMap();
			};

			_options.extentOptions.PropertyHasChanged += (object sender, EventArgs eventArgs) =>
			{
				OnTileProviderChanged();
			};

			_options.extentOptions.defaultExtents.PropertyHasChanged += (object sender, EventArgs eventArgs) =>
			{
				if (Application.isEditor && !Application.isPlaying && IsEditorPreviewEnabled == false)
				{
			...
			_options.placementOptions.PropertyHasChanged += (object sender, EventArgs eventArgs) =>
			{
				SetPlacementStrategy();
				UpdateMap();
			};

			_options.scalingOptions.PropertyHasChanged += (object sender, EventArgs eventArgs) =>
			{
				SetScalingStrategy();
				UpdateMap();
			};
