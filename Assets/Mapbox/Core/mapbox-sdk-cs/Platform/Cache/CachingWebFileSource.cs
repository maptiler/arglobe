namespace Mapbox.Platform.Cache
{
	...
	public class CachingWebFileSource : IFileSource, IDisposable
	{
		...
		)
		{
			...
			//UnityEngine.Debug.Log("Request uri: " + uri);

			// if tile was available propagate to all other caches and return
			if (null != data)
			{
				//UnityEngine.Debug.LogWarning("data cached");
				foreach (var cache in _caches)
				{
			...
			{
				//UnityEngine.Debug.LogError("data not cached");
				...
				return IAsyncRequestFactory.CreateRequest(
					...
					(Response r) =>
					{
						...
						if (!r.HasError && null != r.Data)
						{
							foreach (var cache in _caches)
							{
								//UnityEngine.Debug.LogWarning("Caching: " + tileId);
						...
						else
						{
							if (r.HasError)
							{
								UnityEngine.Debug.LogWarning(
									"Request failed with url: " + r.RequestUrl + "\n" +
									"Error: " + r.ExceptionsAsString);
							}
						}
						callback(r);
					}, timeout);
