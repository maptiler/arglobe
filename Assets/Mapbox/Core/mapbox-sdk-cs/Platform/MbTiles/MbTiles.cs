...
namespace Mapbox.Platform.MbTiles
{
    ...
    {
		public Dictionary<string, SQLTileWorkaround> tiles;

		public SQLiteConnectionWorkaround()
        {
			tiles = new Dictionary<string, SQLTileWorkaround>();
		}

		public void AddTile(string tileKey, SQLTileWorkaround newTile)
        {
	...
	public class MbTilesDb : IDisposable
	{
		...
		public SQLiteConnectionWorkaround SQLiteWorkaround { get; private set; }
...
timestamp   INTEGER NOT NULL,
	PRIMARY KEY(
		...
		private void openOrCreateDb(string dbName)
		{
			...
			SQLiteWorkaround = new SQLiteConnectionWorkaround();
		}
		...
		{
			//Debug.Log("Loading: " + tileId.ToString());
			//Debug.Log(_sqliteWorkaround.tiles.Count);
			SQLTileWorkaround foundTile;
			SQLiteWorkaround.tiles.TryGetValue(tileId.ToString(), out foundTile);
			if(foundTile != null)
				return foundTile.tile_data;
			return null;
		...
		{
			return SQLiteWorkaround.tiles.ContainsKey(tileId.ToString());
			/*return null != _sqlite
				.Table<tiles>()
				.Where(t => t.zoom_level == tileId.Z && t.tile_column == tileId.X && t.tile_row == tileId.Y)
				.FirstOrDefault();*/
